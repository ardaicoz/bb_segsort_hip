...
__device__ inline void exch_intxn_keys(K &k0, int mask, const int bit) {
    K ex_k0, ex_k1;
    ex_k0 = k0;
    ex_k1 = __shfl_xor_sync(BB_FULL_MASK,k0, mask);
...
__device__ inline void exch_intxn_keys(K &k0, K &k1, int mask, const int bit) {
    K ex_k0, ex_k1;
    ex_k0 = k0;
    ex_k1 = __shfl_xor_sync(BB_FULL_MASK,k1, mask);
    CMP_SWP_KEY(K, ex_k0, ex_k1);
    if(bit) EQL_SWP_KEY(K, ex_k0, ex_k1);
    k0 = ex_k0;
    k1 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
}
...
__device__ inline void exch_intxn_keys(K &k0, K &k1, K &k2, K &k3, int mask, const int bit) {
    ...
    k1 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
    ex_k0 = k2;
    ex_k1 = __shfl_xor_sync(BB_FULL_MASK,k3, mask);
    CMP_SWP_KEY(K, ex_k0, ex_k1);
    if(bit) EQL_SWP_KEY(K, ex_k0, ex_k1);
    k2 = ex_k0;
    k3 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
    if(bit) SWP_KEY(K, k0, k2);
    if(bit) SWP_KEY(K, k1, k3);
}
...
__device__ inline void exch_intxn_keys(K &k0, K &k1, K &k2, K &k3, K &k4, K &k5, K &k6, K &k7, int mask, const int bit) {
    ...
    k3 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
    ex_k0 = k4;
    ex_k1 = __shfl_xor_sync(BB_FULL_MASK,k5, mask);
    CMP_SWP_KEY(K, ex_k0, ex_k1);
    if(bit) EQL_SWP_KEY(K, ex_k0, ex_k1);
    k4 = ex_k0;
    k5 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
    ex_k0 = k6;
    ex_k1 = __shfl_xor_sync(BB_FULL_MASK,k7, mask);
    CMP_SWP_KEY(K, ex_k0, ex_k1);
    if(bit) EQL_SWP_KEY(K, ex_k0, ex_k1);
    k6 = ex_k0;
    k7 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
    if(bit) SWP_KEY(K, k0, k6);
    if(bit) SWP_KEY(K, k1, k7);
    if(bit) SWP_KEY(K, k2, k4);
    if(bit) SWP_KEY(K, k3, k5);
...
__device__ inline void exch_intxn_keys(K &k0, K &k1, K &k2, K &k3, K &k4, K &k5, K &k6, K &k7, K &k8, K &k9, K &k10, K &k11, K &k12, K &k13, K &k14, K &k15, int mask, const int bit) {
    ...
    k7 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
    ex_k0 = k8;
    ex_k1 = __shfl_xor_sync(BB_FULL_MASK,k9, mask);
    CMP_SWP_KEY(K, ex_k0, ex_k1);
    if(bit) EQL_SWP_KEY(K, ex_k0, ex_k1);
    k8 = ex_k0;
    k9 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
    ex_k0 = k10;
    ex_k1 = __shfl_xor_sync(BB_FULL_MASK,k11, mask);
    CMP_SWP_KEY(K, ex_k0, ex_k1);
    if(bit) EQL_SWP_KEY(K, ex_k0, ex_k1);
    k10 = ex_k0;
    k11 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
    ex_k0 = k12;
    ex_k1 = __shfl_xor_sync(BB_FULL_MASK,k13, mask);
    CMP_SWP_KEY(K, ex_k0, ex_k1);
    if(bit) EQL_SWP_KEY(K, ex_k0, ex_k1);
    k12 = ex_k0;
    k13 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
    ex_k0 = k14;
    ex_k1 = __shfl_xor_sync(BB_FULL_MASK,k15, mask);
    CMP_SWP_KEY(K, ex_k0, ex_k1);
    if(bit) EQL_SWP_KEY(K, ex_k0, ex_k1);
    k14 = ex_k0;
    k15 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
    if(bit) SWP_KEY(K, k0, k14);
    if(bit) SWP_KEY(K, k1, k15);
    if(bit) SWP_KEY(K, k2, k12);
    if(bit) SWP_KEY(K, k3, k13);
...
__device__ inline void exch_intxn_keys(K &k0, K &k1, K &k2, K &k3, K &k4, K &k5, K &k6, K &k7, K &k8, K &k9, K &k10, K &k11, K &k12, K &k13, K &k14, K &k15, K &k16, K &k17, K &k18, K &k19, K &k20, K &k21, K &k22, K &k23, K &k24, K &k25, K &k26, K &k27, K &k28, K &k29, K &k30, K &k31, int mask, const int bit) {
    ...
    k15 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
    ex_k0 = k16;
    ex_k1 = __shfl_xor_sync(BB_FULL_MASK,k17, mask);
    CMP_SWP_KEY(K, ex_k0, ex_k1);
    if(bit) EQL_SWP_KEY(K, ex_k0, ex_k1);
    k16 = ex_k0;
    k17 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
    ex_k0 = k18;
    ex_k1 = __shfl_xor_sync(BB_FULL_MASK,k19, mask);
    CMP_SWP_KEY(K, ex_k0, ex_k1);
    if(bit) EQL_SWP_KEY(K, ex_k0, ex_k1);
    k18 = ex_k0;
    k19 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
    ex_k0 = k20;
    ex_k1 = __shfl_xor_sync(BB_FULL_MASK,k21, mask);
    CMP_SWP_KEY(K, ex_k0, ex_k1);
    if(bit) EQL_SWP_KEY(K, ex_k0, ex_k1);
    k20 = ex_k0;
    k21 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
    ex_k0 = k22;
    ex_k1 = __shfl_xor_sync(BB_FULL_MASK,k23, mask);
    CMP_SWP_KEY(K, ex_k0, ex_k1);
    if(bit) EQL_SWP_KEY(K, ex_k0, ex_k1);
    k22 = ex_k0;
    k23 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
    ex_k0 = k24;
    ex_k1 = __shfl_xor_sync(BB_FULL_MASK,k25, mask);
    CMP_SWP_KEY(K, ex_k0, ex_k1);
    if(bit) EQL_SWP_KEY(K, ex_k0, ex_k1);
    k24 = ex_k0;
    k25 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
    ex_k0 = k26;
    ex_k1 = __shfl_xor_sync(BB_FULL_MASK,k27, mask);
    CMP_SWP_KEY(K, ex_k0, ex_k1);
    if(bit) EQL_SWP_KEY(K, ex_k0, ex_k1);
    k26 = ex_k0;
    k27 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
    ex_k0 = k28;
    ex_k1 = __shfl_xor_sync(BB_FULL_MASK,k29, mask);
    CMP_SWP_KEY(K, ex_k0, ex_k1);
    if(bit) EQL_SWP_KEY(K, ex_k0, ex_k1);
    k28 = ex_k0;
    k29 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
    ex_k0 = k30;
    ex_k1 = __shfl_xor_sync(BB_FULL_MASK,k31, mask);
    CMP_SWP_KEY(K, ex_k0, ex_k1);
    if(bit) EQL_SWP_KEY(K, ex_k0, ex_k1);
    k30 = ex_k0;
    k31 = __shfl_xor_sync(BB_FULL_MASK,ex_k1, mask);
    if(bit) SWP_KEY(K, k0, k30);
    if(bit) SWP_KEY(K, k1, k31);
    if(bit) SWP_KEY(K, k2, k28);
    if(bit) SWP_KEY(K, k3, k29);
